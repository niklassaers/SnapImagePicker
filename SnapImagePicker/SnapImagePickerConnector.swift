...
public protocol SnapImagePickerConnectorProtocol: class {
    ...
    public static func imagePicker(delegate delegate: SnapImagePickerDelegate) -> UIViewController? {
        ...
        if let viewController = storyboard.instantiateInitialViewController() as? UINavigationController {
            if let snapImagePickerViewController = viewController.viewControllers[0] as? SnapImagePickerViewController {
                snapImagePickerViewController.eventHandler = SnapImagePickerPresenter(view: snapImagePickerViewController)
